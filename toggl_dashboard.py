...
optional arguments:
    -m: write a table with all Toggl time entries and the working days to mysql database
'''
...
working_days_df = define_working_days_table()
...
def sum_worked_hours_by_week(time_entries_extended_df):
    ...
    time_entries_sum_per_week_df = time_entries.groupby(['week'])
    time_entries_sum_per_week_df = time_entries_sum_per_week_df['duration_hours'].agg(np.sum)
    time_entries_sum_per_week_df = pd.DataFrame(time_entries_sum_per_week_df)
